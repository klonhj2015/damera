...
class DameraBase(models.TimestampMixin,
                 models.ModelBase):
    metadata = None
    ...
    def save(self, session=None):
        import damera.db.sqlalchemy.api as db_api

        if session is None:
            session = db_api.get_session()

        super(DameraBase, self).save(session)
...
Base = declarative_base(cls=DameraBase)

class Disk(Base):

    __tablename__ = 'disk'
    __table_args__ = (
        schema.UniqueConstraint('uuid', name='uniq_disk0uuid'),
        table_args()
    )
